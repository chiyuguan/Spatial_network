...
source("mtl_data_with_new_E.R")
source("mtl_graph_data.R")

y = my_data$last_week_2021_new_cases_total; # Modify to total 2021 cases
y3 = my_data3$last_week_2021_new_cases_total; # Modify to total 2021 cases

x = my_data[, c(7:9,14)] %>% st_drop_geometry(); # 33*8 metrix for covariates
# need covariates mean centered and 1 sd scaled,
x <- scale(x, center = TRUE, scale = TRUE)
# Convert the scaled data back to a data frame
x <- as.data.frame(x)

x3 = my_data3[, 7:9] %>% st_drop_geometry(); # 33*8 metrix for covariates
# need covariates mean centered and 1 sd scaled,
...
E = my_data$Expected_count_for_each_boro;
E3 = my_data3$Expected_count_for_each_boro;
...
nbs3 = mungeCARdata4stan_mtl(weights_matrix);
N3 = nbs3$N;
node1_3 = nbs3$node1;
node2_3 = nbs3$node2;
N_edges3 = nbs3$N_edges;
...
bym_model <- cmdstan_model("bym_predictor_plus_offset.stan");
bymW_model <- cmdstan_model("BYM_count_w.stan");
...
# Covariate names
covariate_names <- c("i_visMin", "i_educ_noHS_25_64", "i_LICO", "stops")
...
# Create the scatter plot
plot(mu_values, y3, xlab = "mean of mu_i", ylab = "y_i", main = "M1 scatter Plot of y_i vs mu_i", pch = 19, col = "blue")
...
# Covariate names
covariate_names <- c("i_visMin", "i_educ_noHS_25_64", "i_LICO", "i_stops")
...
# Generate trace plots for selected parameters
mcmc_trace(draws_array, pars = c("tau_theta", theta_names))

mcmc_dens(draws_array, pars = c("beta0", unname(covariate_names)))
mcmc_dens(draws_array, pars = c("tau_theta", theta_names))

### ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ ###
### ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ ###
### M1_C: M1_B+Noninformative Prior
### ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ ###
### ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ ###
bym_M1C_model = cmdstan_model("M1C.stan");
...
# Covariate names
covariate_names <- c("i_visMin", "i_educ_noHS_25_64", "i_LICO")
...
# Extract the mu_i values
summary_mu <- bym_M1C_model_stanfit$summary(variables = c("mu"), ~quantile(.x, probs = c(0.025, 0.5, 0.975)))
mu_values <- summary_mu$`50%` # use the mean of mu

# Create the scatter plot
plot(mu_values, y3, xlab = "mean of mu_i", ylab = "y_i", main = "M2 scatter Plot of y_i vs mu_i", pch = 19, col = "blue")
...
### ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ ###
output_dir <- tempdir() # Or specify a custom directory
bym_model_scot_stanfit = bym_model$sample(
  data = data3,
  parallel_chains = 4,
  refresh=0,
  output_dir = output_dir, # ); default num_iterations=1000
...
# Extract the summary
summary_df <- as.data.frame(bym_model_scot_stanfit$summary(variables = c("lp__", "beta0", "beta")))
...
summary_df <- as.data.frame(bym_model_scot_stanfit$summary(variables = c("SMR[25]", "expbeta0", "expbeta")))
for (i in 1:nrow(param_map2)) { #i=3
  summary_df$variable <- gsub(param_map2$parameter[i], param_map2$covariate[i], summary_df$variable, fixed = TRUE)
} #treats the pattern as a fixed string rather than a regular expression. This avoids any issues with special characters like [ ]
print(summary_df)
...
## For mu
bym_model_scot_stanfit$summary(variables = c("mu"));
bym_model_scot_stanfit$summary(variables = c("mu"),
                               ~quantile(.x, probs = c(0.025, 0.5, 0.975)));

# Extract the mu_i values
summary_mu <- bym_model_scot_stanfit$summary(variables = c("mu"), ~quantile(.x, probs = c(0.025, 0.5, 0.975)))
mu_values <- summary_mu$`50%` # use the mean of mu

# Create the scatter plot
plot(mu_values, y3, xlab = "mean of mu_i", ylab = "y_i", main = "M3 scatter Plot of y_i vs mu_i", pch = 19, col = "blue")

bym_model_scot_stanfit$summary(variables = c("expsigma_theta", "expsigma_phi"))
bym_model_scot_stanfit$summary(variables = c("expsigma_theta", "expsigma_phi"),~quantile(.x, probs = c(0.025, 0.5, 0.975)));

library(posterior)
library(ggplot2)
# For full model
# Extract posterior samples
posterior_samples <- bym_model_scot_stanfit$draws(variables = 'mu')
...
## For WAIC & LOO
waic_result <- waic(as.matrix(bym_model_scot_stanfit$draws("log_lik")))
print(waic_result)

loo_result <- loo(as.matrix(bym_model_scot_stanfit$draws("log_lik")))
print(loo_result)
...
# Extract the draws
draws <- bym_model_scot_stanfit$draws()
...
# Generate trace plots for selected parameters
mcmc_trace(draws_array, pars = c("tau_theta", "tau_phi", theta_names))

mcmc_dens(draws_array, pars = c("beta0", unname(covariate_names)))
mcmc_dens(draws_array, pars = c("tau_theta", "tau_phi", phi_names))
...
# Create the scatter plot
plot(mu_values, y, xlab = "mean of mu_i", ylab = "y_i", main = "M4 scatter Plot of y_i vs mu_i", pch = 19, col = "blue")
...
# For full model
# Extract posterior samples
posterior_samples <- bym_model_scot_stanfit$draws(variables = 'SMR')
...
mcmc_trace(draws_array, pars = c("tau_theta", "tau_phi", "theta[25]", "phi[33]"))

mcmc_dens(draws_array, pars = c("beta0", unname(covariate_names)))
mcmc_dens(draws_array, pars = c("tau_theta", "tau_phi", "theta[25]", "phi[33]"))

#plot SMR
# Extract the SMR row
smr_data <- summary_df[summary_df$variable == "SMR", ]
# Load ggplot2
library(ggplot2)

# Create the error bar plot
ggplot(smr_data, aes(x = variable, y = `50%`)) +
  geom_point() +
  geom_errorbar(aes(ymin = `2.5%`, ymax = `97.5%`), width = 0.2) +
  labs(title = "Error Bar Plot for SMR",
       x = "SMR",
       y = "Estimate") +
  theme_minimal()
...
data5 = list(N=N,
            ...
            W=adjacency_matrix);
output_dir <- tempdir() # Or specify a custom directory
bymW_model_scot_stanfit = bymW_model$sample(
  data = data5,
  parallel_chains = 4,
  refresh=0,
  output_dir = output_dir, # ); default num_iterations=1000
...
# Extract the summary
summary_df <- as.data.frame(bymW_model_scot_stanfit$summary(variables = c("lp__", "beta0", "beta")))
...
summary_df <- as.data.frame(bymW_model_scot_stanfit$summary(variables = c("SMR[25]", "expbeta0", "expbeta")))
for (i in 1:nrow(param_map2)) { #i=3
  summary_df$variable <- gsub(param_map2$parameter[i], param_map2$covariate[i], summary_df$variable, fixed = TRUE)
} #treats the pattern as a fixed string rather than a regular expression. This avoids any issues with special characters like [ ]
print(summary_df)
...
## For mu
bymW_model_scot_stanfit$summary(variables = c("mu"));
bymW_model_scot_stanfit$summary(variables = c("mu"),
                               ~quantile(.x, probs = c(0.025, 0.5, 0.975)));

# Extract the mu_i values
summary_mu <- bymW_model_scot_stanfit$summary(variables = c("mu"), ~quantile(.x, probs = c(0.025, 0.5, 0.975)))
mu_values <- summary_mu$`50%` # use the mean of mu

# Create the scatter plot
plot(mu_values, y, xlab = "mean of mu_i", ylab = "y_i", main = "M5 scatter Plot of y_i vs mu_i", pch = 19, col = "blue")

bymW_model_scot_stanfit$summary(variables = c("expsigma_theta", "expsigma_phi"))
bymW_model_scot_stanfit$summary(variables = c("expsigma_theta", "expsigma_phi"),~quantile(.x, probs = c(0.025, 0.5, 0.975)));

library(posterior)
library(ggplot2)
# For full model
# Extract posterior samples
posterior_samples <- bymW_model_scot_stanfit$draws(variables = 'mu')
...
## For WAIC & LOO
waic_result <- waic(as.matrix(bymW_model_scot_stanfit$draws("log_lik")))
print(waic_result)

loo_result <- loo(as.matrix(bymW_model_scot_stanfit$draws("log_lik")))
print(loo_result)
...
# Extract the draws
draws <- bymW_model_scot_stanfit$draws()
