...
mtl_Boro_Total_df <- as.data.frame(mtl_Boro_Total)

# Perform the merge
mtl_Boro_Total_df <- mtl_Boro_Total_df %>% 
  mutate(Expected_count_for_each_boro = ifelse(Expected_count_for_each_boro == 0, 1, Expected_count_for_each_boro))
merged_df <- merge(mtl_Boro_Total_df, Predictor_var, by = "NOM")
